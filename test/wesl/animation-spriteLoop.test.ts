...
import { createSampler, getGPUDevice } from "wesl-test";
import { createSimpleSpriteSheet } from "./spriteTestUtil.ts";
import { expectCloseTo, lygiaTestFragment } from "./testUtil.ts";
...
test("spriteLoop - index 0", async () => {
  ...
  const result = await lygiaTestFragment(src, {
...
test("spriteLoop - time wrapping", async () => {
  ...
  const resultTime0 = await lygiaTestFragment(srcTime0, {
    textureFormat: "rgba32float",
    size: [256, 256],
    inputTextures: [{ texture: spriteTex, sampler }],
  });

  const resultTime16 = await lygiaTestFragment(srcTime16, {
    textureFormat: "rgba32float",
    size: [256, 256],
    inputTextures: [{ texture: spriteTex, sampler }],
  });
